...
struct ByteUtil {
  ...
  template <class Target, size_t NbBytes> static Target convertTo(std::span<std::byte, NbBytes> source);
};

template <class Target, size_t NbBytes> Target ByteUtil::convertTo(std::span<std::byte, NbBytes> source) {
